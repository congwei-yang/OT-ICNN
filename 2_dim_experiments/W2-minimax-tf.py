...
parser.add_argument('--DATASET_X', type=str, default='circ_disc', help='which dataset to use for X')
...
parser.add_argument('--TRIAL', type=int, default=3, help='the trail no.')
...
parser.add_argument('--ITERS', type=int, default=2000, help='number of iterations of training')
...
def main():
    ...
    for i in range(len(sample_size)):
        X_test_set.append(next(sample_data_gen(opt.DATASET_X, sample_size[i], opt.SCALE, opt.VARIANCE)))
        Y_test_set.append(next(sample_data_gen(opt.DATASET_Y, sample_size[i], opt.SCALE, opt.VARIANCE)))
    ...
    with tf.Session() as sess:
        ...
        for i in range(len(sample_size)):
            X_test_new = X_test_set[i]
            Y_test_new = Y_test_set[i]
            f_txt = open("W2_ICNN_Trial_"+str(opt.TRIAL), "a+")
            f_txt.write("Final Wasserstein distance: {0}".format(compute_OT.compute_W2(X_test_new, Y_test_new)) + " ")
            f_txt.close()
    ...
    for i in range(len(sample_size)):
        X_test_new2 = X_test_set[i]
        Y_test_new2 = Y_test_set[i]
        f2_txt = open("W2_Actual_Trial_"+str(opt.TRIAL), "a+")
        f2_txt.write("Actual Wasserstein distance: {0}".format(python_OT(X_test_new2, Y_test_new2, sample_size[i])) + " ")
        f2_txt.close()
        f3_txt = open("W2_Skh_Trial_" + str(opt.TRIAL), "a+")
        f3_txt.write("Skh Wasserstein distance: {0}".format(python_skh(X_test_new2, Y_test_new2, sample_size[i], reg = 1/20)) + " ")
        f3_txt.close()
#    for j in range(len(sample_size)):
#        X_test_new3 = X_test_set[i]
#        Y_test_new3 = Y_test_set[i]
#        f3_txt = open("W2_Skh_Trial_"+str(opt.TRIAL), "a+")
#        f3_txt.write("Skh Wasserstein distance: {0}".format(python_OT(X_test_new3, Y_test_new3, sample_size[i])) + " ")
#        f3_txt.close()
...
def python_skh(X,Y,n,reg):
    a, b = np.ones((n,)) / n, np.ones((n,)) / n  # uniform distribution on samples
    # loss matrix
    M = ot.dist(Y, X)
    scaling = M.max()
    M /= M.max()
    G0 = ot.sinkhorn(a, b, M, reg, numItermax = 10000000)
    return 0.5 * scaling*sum(sum(G0*M))
